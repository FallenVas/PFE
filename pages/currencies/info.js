...
const info = () => {
  ...
  useEffect(() => {
    ...
    const intiliazeData = async () => {
      ...
      "autosize": true,
      "symbol": "${symbole!=='USDT'?'BINANCE:'+symbole+'USDT':'COINBASE:USDTUSD'}",
      "interval": "D",
      "timezone": "Etc/UTC",
      "theme": "dark",
      "style": "1",
      ...
      {coin && (
        ...
        <div className="grid grid-cols-2 w-full border-b border-solid border-white border-opacity-10 pb-4">
          ...
          <div className="inline-flex flex-col gap-2">
            <p className="text-white opacity-70 text-xs">{coin.name} Price ({coin.symbol})</p>
            <div className="flex gap-x-3 items-center">
              <p className="text-4xl font-bold">${coin.quote.USD.price.toFixed(2)}</p>
              <div className={`rounded ${coin.quote.USD.percent_change_24h>0?'bg-green-500':'bg-red-500'} h-2/3 px-2 text-sm font-bold flex gap-1 items-center text-white`}>
                {coin.quote.USD.percent_change_24h>0?<FaChevronUp/>:<FaChevronDown/>}
